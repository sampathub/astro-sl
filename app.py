...
# --- Config & AI ---
st.set_page_config(page_title="AstroPro SL v16", page_icon="☸️", layout="wide")
    ...
    for i in range(1, 4):
        ...
        if key:
            try:
                ...
                return model.generate_content(f"ඔබ ප්‍රවීණ ජ්‍යොතිෂවේදියෙකි. මෙම දත්ත වල පලාපල සිංහලෙන් ලියන්න: {data}").text
            except: continue
    return "AI සේවාව කාර්යබහුලයි."

# --- Data Tables ---
DISTRICTS = {"කොළඹ":(6.9271,79.8612),"ගම්පහ":(7.0840,79.9927),"කළුතර":(6.5854,79.9607),"මහනුවර":(7.2906,80.6337),"මාතලේ":(7.4675,80.6234),"නුවරඑළිය":(6.9497,80.7891),"ගාල්ල":(6.0535,80.2210),"මාතර":(5.9549,80.5550),"හම්බන්තොට":(6.1246,81.1245),"යාපනය":(9.6615,80.0255),"කිලිනොච්චිය":(9.3854,80.3921),"මන්නාරම":(8.9810,79.9044),"වවුනියාව":(8.7542,80.4982),"මුලතිව්":(9.2671,80.8143),"මඩකලපුව":(7.7102,81.6924),"අම්පාර":(7.2843,81.6747),"ත්‍රිකුණාමලය":(8.5711,81.2335),"කුරුණෑගල":(7.4863,80.3647),"පුත්තලම":(8.0330,79.8257),"අනුරාධපුරය":(8.3114,80.4037),"පොළොන්නරුව":(7.9403,81.0188),"බදුල්ල":(6.9934,81.0550),"මොණරාගල":(6.8719,81.3512),"රත්නපුරය":(6.7056,80.3847),"කෑගල්ල":(7.2513,80.3464)}
...
if st.button("කේන්ද්‍රය ගණනය කරන්න"):
    if u_name:
        lat, lon = DISTRICTS[u_city]; jd = swe.julday(u_dob.year, u_dob.month, u_dob.day, u_h+u_m/60.0+u_s/3600.0-5.5)
        swe.set_sid_mode(swe.SIDM_LAHIRI)
        houses, ascmc = swe.houses_ex(jd, lat, lon, b'P', swe.FLG_SIDEREAL); lag_idx = int(ascmc[0]/30)
        
        pos_map = {i: [] for i in range(12)}; moon_lon = 0
        for n, i in {"රවි":0,"සඳු":1,"කුජ":4,"බුධ":2,"ගුරු":5,"සිකුරු":3,"ශනි":6,"රාහු":10}.items():
            res, _ = swe.calc_ut(jd, i, swe.FLG_SIDEREAL)
            if i==1: moon_lon = res[0]
            pos_map[int(res[0]/30)].append(n)
            
        n_idx = int(moon_lon/(360/27)); n_idx = min(n_idx, 26)
        d_idx = n_idx % 9; bal = (1-((moon_lon-(n_idx*(360/27)))/(360/27))) * D_YEARS[d_idx]
        
        st.header(f"📊 {u_name} මහතාගේ වාර්තාව")
        col1, col2 = st.columns([1,1])
        with col1:
            img = Image.new('RGB',(600,600),(255,255,255)); d = ImageDraw.Draw(img)
            for i in range(5): d.line([(i*150,0),(i*150,600)],"black",3); d.line([(0,i*150),(600,i*150)],"black",3)
            ...
            for r, (x,y) in g_m.items():
                if r==lag_idx: d.text((x+65,y+10),"L",(255,0,0))
                for i,p in enumerate(pos_map[r]): d.text((x+25,y+40+(i*25)),{"රවි":"Sun","සඳු":"Moon","කුජ":"Mars","බුධ":"Merc","ගුරු":"Jup","සිකුරු":"Ven","ශනි":"Sat","රාහු":"Rahu"}.get(p,p),"black")
        ...
        with col2:
            ...
            cy = u_dob.year + bal
            for i in range(1, 4):
                idx = (d_idx + i) % 9; st.write(f"• {D_LORDS[idx]}: {int(cy)} සිට {int(cy+D_YEARS[idx])} දක්වා"); cy += D_YEARS[idx]
        st.session_state['res'] = {"name":u_name,"lagna":RA_NAMES[lag_idx],"nak":NAK_NAMES[n_idx]}
    else: st.error("නම ඇතුළත් කරන්න.")

if 'res' in st.session_state:
    if st.button("🤖 AI පලාපල ලබාගන්න"):
        st.write(get_ai_prediction(str(st.session_state['res'])))
